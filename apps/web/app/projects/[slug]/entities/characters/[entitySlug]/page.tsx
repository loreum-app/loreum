...
export default function CharacterDetailPage() {
  ...
  const startEditing = () => {
    ...
    setEditStatus(entity.character?.status ?? "");
    setEditSpecies(entity.character?.species ?? "");
    setEditAge(entity.character?.age ?? "");
    setEditRole(entity.character?.role ?? "");
    setEditing(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (updated.slug !== params.entitySlug) {
        router.replace(
          `/projects/${params.slug}/entities/characters/${updated.slug}`,
        );
      }
    } catch {
  ...
  const handleDelete = async () => {
    if (!entity || !confirm("Delete this character?")) return;
    try {
      await api(`/projects/${params.slug}/entities/${entity.slug}`, {
        method: "DELETE",
      });
      router.push(`/projects/${params.slug}/entities/characters`);
    } catch {
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <Select
                    value={editSpecies}
                    onValueChange={(v) => v && setEditSpecies(v)}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    placeholder="Age or age description"
                  />
                </div>
                <div className="space-y-1">
            ...
            <Card>
              ...
              <CardContent>
                <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                  ...
                  {entity.character.age && (
                    <div>
                      <dt className="text-muted-foreground">Age</dt>
                      <dd className="font-medium">{entity.character.age}</dd>
                    </div>
                  )}
                  {entity.character.role && (
                    <div>
                      <dt className="text-muted-foreground">Narrative Role</dt>
                      <dd className="font-medium capitalize">
                        {entity.character.role}
                      </dd>
                    </div>
