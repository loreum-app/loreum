...
Tracked tasks for the MVP release and beyond. Check items off as they're completed.

## MVP - Ship It

These need to be done before deploying to production.
...
- [ ] Create open source repo (fresh git history)
- [ ] Deploy production at loreum.app (PM2 + Cloudflare Tunnel)
- [ ] Deploy staging at staging.loreum.app
...
- [ ] MCP tool: `get_style_guide` (returns base guide + scene overrides + character voices)
...
Features planned but not blocking the initial release.
